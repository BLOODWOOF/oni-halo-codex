...
function categoriesWithCounts() {
  ...
  return [['All Files', entries.length], ...Array.from(counts.entries())];
}
...
function renderCategories() {
  $('categoryList').innerHTML = categoriesWithCounts().map(([name, count]) => `
    <button class="cat-btn ${name === selectedCategory ? 'active' : ''}" data-category="${escapeHtml(name)}">
      ${escapeHtml(name)} <span class="cat-count">${count}</span>
    </button>
  `).join('');
...
function filteredEntries() {
  ...
  if (selectedCategory !== 'All Files') list = list.filter(e => e.category === selectedCategory);
...
function renderFileList() {
  ...
  if (!list.length) {
    $('fileList').innerHTML = '<div class="empty">No matching archive files found.</div>';
    return;
  }
...
function getVisualBundle(e) {
  ...
  const byTitle = [
    ...
    { match: ['source','index','canon','reference','glossary','checklist','cross reference','micro lore'], banner: 'assets/unsc-banner.svg', caption: 'Reference sector — orientation materials, indexes, and source-routing aids for the public archive.', diagrams: [
  ...
  return { banner: 'assets/unsc-banner.svg', caption: 'Public archive node — browse the Halo lore encyclopedia by chronology, reference sector, or source index.', diagrams: [['assets/timeline-overview.svg', 'Master chronology', 'The major eras covered by this archive.'], ['assets/source-ecosystem.svg', 'Source ecosystem', 'The types of material synthesized into the archive.']] };
}
